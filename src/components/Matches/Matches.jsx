import React from "react";
import image1 from "../../assets/owuigawygwwzwtpydh6f.avif";
import { Link } from "react-router-dom";

const Matches = () => {
    return (
        <section>
            <div class="flex items-center justify-center h-auto mx-8 lg:mx-28">
                <div class="text-center">
                    <h1 class="text-4xl font-bold">
                        UPCOMING <span class="text-yellow-500">CRICKET </span>
                        SPOTLIGHT IN THIS MONTH
                    </h1>
                    <p class="mt-4 text-gray-600 text-lg">
                        Feel the thrill of seeing a global sporting event in one
                        of the world's most incredible cities. Headlining the
                        calendar is the Dubai World Cup
                    </p>
                </div>
            </div>

            <div className="mx-8 lg:mx-28 flex justify-start gap-1 sm:gap-3 items-center my-5">
                <div className="cursor-pointer border-2 px-2 sm:px-3 py-1 border-green-700 bg-green-700 text-white duration-200 transition-all">
                    Upcoming
                </div>
                <div className="cursor-pointer border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                    Live
                </div>
                <div className="cursor-pointer border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                    Completed
                </div>
                <Link
                    to="/tournaments"
                    class="border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all"
                >
                    View All
                </Link>
            </div>

            <div class="flex md:flex-row flex-col gap-10 my-10 mx-8 lg:mx-28 pb-5 border-b-2 border-blue-300">
                <div class="h-[250px] flex justify-center items-center">
                    <img
                        class="h-[250px] w-full md:w-[250px]"
                        src={image1}
                        alt="image"
                    />
                </div>

                <div>
                    <div class="inline-block bg-green-600 px-2 py-1 text-white">
                        Sept 29 - Oct 5
                    </div>
                    <div class="my-3">
                        <h2 class="text-xl lg:text-4xl font-bold">
                            ICC Champions Trophy 2017, Australia
                        </h2>
                        <p>
                            The indoor sports mania is back again offering all
                            sorts of indoor sports in the summer.
                        </p>
                    </div>
                    <div class="flex gap-1 sm:gap-5">
                        <button class="border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                            Register Now
                        </button>
                        <button class="border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                            Ticket Booking
                        </button>
                        <button class="border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                            Read More
                        </button>
                    </div>
                </div>
            </div>
            <div class="flex md:flex-row flex-col gap-10 my-10 mx-8 lg:mx-28 pb-5 border-b-2 border-blue-300">
                <div class="h-[250px] flex justify-center items-center">
                    <img
                        class="h-[250px] w-full md:w-[250px]"
                        src={image1}
                        alt="image"
                    />
                </div>

                <div>
                    <div class="inline-block bg-green-600 px-2 py-1 text-white">
                        Sept 29 - Oct 5
                    </div>
                    <div class="my-3">
                        <h2 class="text-xl lg:text-4xl font-bold">
                            ICC Champions Trophy 2017, Australia
                        </h2>
                        <p>
                            The indoor sports mania is back again offering all
                            sorts of indoor sports in the summer.
                        </p>
                    </div>
                    <div class="flex gap-1 sm:gap-5">
                        <button class="border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                            Register Now
                        </button>
                        <button class="border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                            Ticket Booking
                        </button>
                        <button class="border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                            Read More
                        </button>
                    </div>
                </div>
            </div>
            <div class="flex md:flex-row flex-col gap-10 my-10 mx-8 lg:mx-28 pb-5 border-b-2 border-blue-300">
                <div class="h-[250px] flex justify-center items-center">
                    <img
                        class="h-[250px] w-full md:w-[250px]"
                        src={image1}
                        alt="image"
                    />
                </div>

                <div>
                    <div class="inline-block bg-green-600 px-2 py-1 text-white">
                        Sept 29 - Oct 5
                    </div>
                    <div class="my-3">
                        <h2 class="text-xl lg:text-4xl font-bold">
                            ICC Champions Trophy 2017, Australia
                        </h2>
                        <p>
                            The indoor sports mania is back again offering all
                            sorts of indoor sports in the summer.
                        </p>
                    </div>
                    <div class="flex gap-1 sm:gap-5">
                        <button class="border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                            Register Now
                        </button>
                        <button class="border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                            Ticket Booking
                        </button>
                        <button class="border-2 border-green-300 px-2 sm:px-3 py-1 hover:border-green-700 hover:bg-green-700 hover:text-white duration-200 transition-all">
                            Read More
                        </button>
                    </div>
                </div>
            </div>
        </section>
    );
};

export default Matches;
